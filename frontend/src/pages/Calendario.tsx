  ...
  return (
    <div className="max-w-7xl mx-auto pb-12">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 gap-4">
        <div>
          <h2 className="text-3xl font-semibold text-gray-800 mb-1">Calendario de entregas</h2>
          <p className="text-gray-500">Visualiza tus tareas pendientes en el mes</p>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-10">
        <div className="flex justify-between items-center mb-8">
          <button onClick={irAMesAnterior} className="p-2 hover:bg-gray-100 rounded-xl text-gray-500 transition-colors" title="Mes anterior" aria-label="Mes anterior">
            <ChevronLeft size={24} />
          </button>
          <h3 className="text-2xl font-medium text-gray-800">{nombresMeses[mesCalendario]} {anioCalendario}</h3>
          <button onClick={irAMesSiguiente} className="p-2 hover:bg-gray-100 rounded-xl text-gray-500 transition-colors" title="Mes siguiente" aria-label="Mes siguiente">
            <ChevronRight size={24} />
          </button>
        </div>

        <div className="grid grid-cols-7 gap-2 sm:gap-4 text-center mb-4">
          {diasSemanaCorto.map(dia => <div key={dia} className="text-sm font-semibold text-gray-500">{dia}</div>)}
        </div>
          ...
          {Array.from({ length: diasEnMes }).map((_, i) => {
            ...
            return (
              <div 
                ...
                className="min-h-20 sm:min-h-30 border border-black/5 flex flex-col py-2 px-1 sm:px-3 bg-fondo-lateral/10 rounded-2xl cursor-pointer hover:bg-fondo-lateral/30 transition-colors overflow-hidden"
              >
                <span className="text-sm font-medium text-gray-800 mb-1">{dia}</span>
                <div className="flex flex-col gap-1 overflow-y-auto custom-scrollbar flex-1">
                  {tareasDelDia.map(tarea => {
                    ...
                    return (
                      <div 
                        key={tarea.id_tarea} 
                        className={`text-[10px] sm:text-xs truncate px-2 py-1 rounded-md text-gray-900 font-normal ${tarea.completada ? 'opacity-40 line-through' : ''}`}
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {pendientesDia.map(tarea => {
                            const materia = materias.find(m => m.id_materia === tarea.id_materia);
                            return (
                              <div key={tarea.id_tarea} className="flex items-center gap-3 p-2 border border-gray-100 rounded-lg bg-white">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {completadasDia.map(tarea => {
                            const materia = materias.find(m => m.id_materia === tarea.id_materia);
                            return (
                              <div key={tarea.id_tarea} className="flex items-center gap-3 p-2 border border-gray-100 rounded-lg opacity-60 bg-white">
                                <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: materia?.color || '#a0add3' }}></div>
                                <div className="flex-1 min-w-0">
