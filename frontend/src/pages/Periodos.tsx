...
function Periodos() {
  ...
  // Ajustamos el 'valor' exactamente a como lo pide tu base de datos (Lun, Mar, Mie...)
  ...
  const guardarPeriodo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (idPeriodoEditando) {
        await api.put(`/periodos/${idPeriodoEditando}`, datosPeriodo);
      } else {
        await api.post('/periodos/', datosPeriodo);
      }
      setModalPeriodoAbierto(false);
      cargarDatos();
    } catch (error: any) {
      alert(`Error: ${error.response?.data?.error || 'Error al guardar el periodo'}`);
    }
  };
  ...
  const borrarPeriodo = async (id: number) => {
    ...
    try {
      ...
      alert('Error al intentar eliminar el periodo.');
    }
  };

  const abrirModalNuevoPeriodo = () => {
    setIdPeriodoEditando(null);
    setNombrePeriodo('');
    setFechaInicio('');
    setFechaFin('');
  ...
  const abrirModalEditarPeriodo = (periodo: Periodo) => {
    setIdPeriodoEditando(periodo.id_periodo);
    setNombrePeriodo(periodo.nombre);
  ...
  const guardarMateria = async (e: React.FormEvent) => {
    ...
    try {
      const datosMateria = {
        ...
        id_periodo: idPeriodoParaMateria,
        color: colorMateria
      };

      if (idMateriaEditando) {
        await api.put(`/materias/${idMateriaEditando}`, datosMateria);
        if (materiaVista && materiaVista.id_materia === idMateriaEditando) {
          setMateriaVista({ ...materiaVista, ...datosMateria } as Materia);
        }
      } else {
      ...
      alert(`Error: ${error.response?.data?.error || 'Error al guardar la materia'}`);
    }
  };
  ...
  const abrirModalNuevaMateria = (id_periodo: number) => {
    ...
    setIdPeriodoParaMateria(id_periodo);
    setNombreMateria('');
    setProfesorMateria('');
    setColorMateria('#a0add3');
    setModalMateriaAbierto(true);
  };

  const abrirModalEditarMateria = (materia: Materia) => {
    setIdMateriaEditando(materia.id_materia);
    setIdPeriodoParaMateria(materia.id_periodo);
    setNombreMateria(materia.nombre);
    setProfesorMateria(materia.profesor || '');
    setColorMateria(materia.color || '#a0add3');
    setModalMateriaAbierto(true);
  };
  ...
  const guardarHorario = async (e: React.FormEvent) => {
    ...
    try {
      const datosHorario = {
        id_materia: materiaVista?.id_materia,
        dia_semana: diaSemana,
        hora_inicio: horaInicio,
        hora_fin: horaFin
      };

      if (idHorarioEditando) {
        await api.put(`/horarios/${idHorarioEditando}`, datosHorario);
      } else {
        await api.post('/horarios/', datosHorario);
      }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex justify-between items-end mb-10">
        ...
        <button 
          onClick={abrirModalNuevoPeriodo}
          className="flex items-center gap-2 bg-boton-activo text-texto-fuerte px-6 py-3 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_20px_rgba(0,0,0,0.1)] transition-all hover:scale-105 font-medium"
        >
          <Plus size={18} />
          Nuevo periodo
        </button>
      ...
      {cargando ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {periodos.map((periodo) => {
            ...
            return (
              <div key={periodo.id_periodo} className="bg-white rounded-4xl shadow-[0_8px_30px_rgba(0,0,0,0.08)] overflow-hidden flex flex-col transition-all hover:shadow-[0_12px_40px_rgba(0,0,0,0.12)] border border-black/5">
                
                <div className="bg-fondo-lateral px-6 py-5 flex justify-between items-center">
                  <h3 className="text-xl font-bold text-texto-fuerte truncate pr-2">
                    Periodo: {periodo.nombre}
                  </h3>
                  <div className="flex gap-1 shrink-0">
                    <button 
                      onClick={() => abrirModalEditarPeriodo(periodo)}
                      aria-label={`Editar periodo ${periodo.nombre}`}
                      title="Editar periodo"
                      className="p-2 text-texto-suave hover:bg-white/40 rounded-xl hover:text-texto-fuerte transition-colors"
                    >
                      <Edit2 size={16} />
                    </button>
                    <button 
                      onClick={() => borrarPeriodo(periodo.id_periodo)}
                      aria-label={`Eliminar periodo ${periodo.nombre}`}
                      title="Eliminar periodo"
                      className="p-2 text-texto-suave hover:bg-red-400/30 rounded-xl hover:text-red-600 transition-colors"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
                ...
                <div className="p-6 flex flex-col flex-1 bg-white">
                  ...
                  <div className="bg-white shadow-[0_4px_20px_rgba(0,0,0,0.04)] rounded-2xl p-4 flex flex-col flex-1 border border-black/5">
                      ...
                      <div className="space-y-1 mb-5 flex-1">
                        {materiasDelPeriodo.map((materia) => (
                          <button 
                            ...
                            onClick={() => setMateriaVista(materia)}
                            title={`Ver detalles de ${materia.nombre}`}
                            aria-label={`Ver detalles de ${materia.nombre}`}
                            className="w-full flex items-center gap-3 text-sm text-texto-fuerte p-2 rounded-xl hover:bg-black/5 transition-colors text-left"
                          >
                            <div 
                    ...
                    <button 
                      onClick={() => abrirModalNuevaMateria(periodo.id_periodo)}
                      className="w-full flex items-center justify-center gap-2 py-3 mt-auto bg-boton-activo text-texto-fuerte rounded-xl font-medium shadow-sm hover:scale-[1.02] transition-transform text-sm"
                    >
                      <Plus size={16} />
                      Agregar materia
                    </button>
      ...
      {materiaVista && (
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/10 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-4xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] overflow-hidden flex flex-col border border-black/5 relative">
            
            <button 
              onClick={() => setMateriaVista(null)}
              className="absolute top-6 right-6 text-texto-fuerte bg-white/50 backdrop-blur-md hover:bg-white p-2 rounded-full transition-colors z-10 shadow-sm"
              title="Cerrar vista"
            ...
            <div className="px-8 pt-10 pb-8 flex flex-col relative" style={{ backgroundColor: materiaVista.color || '#a0add3' }}>
              <div className="bg-white/30 backdrop-blur-sm self-start px-3 py-1 rounded-lg text-xs font-bold text-texto-fuerte uppercase tracking-wider mb-4 shadow-sm">
                {periodos.find(p => p.id_periodo === materiaVista.id_periodo)?.nombre || 'Periodo'}
              </div>
              <h3 className="text-3xl font-bold text-texto-fuerte mb-2 leading-tight">
                {materiaVista.nombre}
              </h3>
              <p className="text-texto-fuerte/80 font-medium flex items-center gap-2">
            ...
            <div className="bg-fondo-principal px-8 py-4 flex gap-3 border-b border-black/5">
              <button 
                onClick={() => abrirModalEditarMateria(materiaVista)}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-white text-texto-suave hover:text-texto-fuerte rounded-xl font-medium shadow-sm transition-all text-sm border border-black/5"
              >
                <Edit2 size={16} />
                Editar materia
              </button>
              <button 
                onClick={() => borrarMateria(materiaVista.id_materia)}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-white text-texto-suave hover:text-red-500 rounded-xl font-medium shadow-sm transition-all text-sm border border-black/5"
              >
                <Trash2 size={16} />
                Eliminar materia
              </button>
            ...
            <div className="p-8 bg-white flex-1">
              ...
              <div className="space-y-3 mb-6">
                {horarios.filter(h => h.id_materia === materiaVista.id_materia).length === 0 ? (
                  <p className="text-sm text-texto-suave/70 italic p-4 bg-fondo-principal rounded-2xl border border-black/5 text-center">
                    Aún no has registrado horarios para esta clase.
                  </p>
                ) : (
                  horarios
                    .filter(h => h.id_materia === materiaVista.id_materia)
                    .map(horario => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => abrirModalEditarHorario(horario)}
                          title="Editar horario"
                          aria-label={`Editar horario del día ${nombreDia}`}
                          className="w-full bg-fondo-principal hover:bg-fondo-lateral border border-black/5 p-4 rounded-2xl flex justify-between items-center transition-colors text-left group"
                        >
                          <span className="font-semibold text-texto-fuerte text-lg capitalize">
                            {nombreDia}
                          </span>
                          <div className="flex items-center gap-4">
                            <span className="text-texto-suave font-medium">
              ...
              <button 
                ...
                className="w-full flex items-center justify-center gap-2 py-3 bg-boton-activo text-texto-fuerte rounded-xl font-medium shadow-sm hover:scale-[1.02] transition-transform text-sm"
              >
                <Plus size={16} />
                Agregar horario
              </button>
      ...
      {modalPeriodoAbierto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/10 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-4xl p-10 shadow-[0_20px_50px_rgba(0,0,0,0.1)] relative border border-black/5">
            <button 
              onClick={() => setModalPeriodoAbierto(false)} 
              title="Cerrar" 
              aria-label="Cerrar modal de periodo" 
            ...
            <form onSubmit={guardarPeriodo} className="space-y-6">
              ...
              <div className="pt-4 flex gap-3">
                <button type="submit" className="flex-1 bg-texto-fuerte text-white py-4 rounded-2xl font-medium shadow-lg hover:scale-[1.02] transition-transform">Guardar</button>
                <button type="button" onClick={() => setModalPeriodoAbierto(false)} className="px-6 py-4 rounded-2xl font-medium text-texto-suave hover:bg-black/5 transition-colors">Cancelar</button>
              </div>
            </form>
          </div>
        </div>
            ...
            <form onSubmit={guardarMateria} className="space-y-6">
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                <div className="col-span-1">
                  ...
                  <div className="w-full h-14 bg-fondo-principal p-2 rounded-2xl flex justify-center items-center">
                    <input 
                      id="colorMateria"
                      title="Seleccionar color"
                      type="color" 
                      value={colorMateria} 
                      onChange={(e) => setColorMateria(e.target.value)} 
              ...
              <div className="pt-4 flex gap-3">
                <button type="submit" className="flex-1 bg-texto-fuerte text-white py-4 rounded-2xl font-medium shadow-lg hover:scale-[1.02] transition-transform">Guardar</button>
                <button type="button" onClick={() => setModalMateriaAbierto(false)} className="px-6 py-4 rounded-2xl font-medium text-texto-suave hover:bg-black/5 transition-colors">Cancelar</button>
              </div>
            </form>
          </div>
        </div>
            ...
            <form onSubmit={guardarHorario} className="space-y-6">
              ...
              <div className="pt-4 flex flex-col gap-3">
                <div className="flex gap-3">
                  <button type="submit" className="flex-1 bg-texto-fuerte text-white py-4 rounded-2xl font-medium shadow-lg hover:scale-[1.02] transition-transform">Guardar</button>
                  <button type="button" onClick={() => setModalHorarioAbierto(false)} className="px-6 py-4 rounded-2xl font-medium text-texto-suave hover:bg-black/5 transition-colors">Cancelar</button>
                </div>
                {idHorarioEditando && (
                  <button type="button" onClick={() => { borrarHorario(idHorarioEditando); setModalHorarioAbierto(false); }} className="w-full py-4 rounded-2xl font-medium text-red-500 hover:bg-red-50 transition-colors">
                    Eliminar horario
                  </button>
                )}
              </div>
